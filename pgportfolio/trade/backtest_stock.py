...
import numpy as np
from pgportfolio.trade import trader_stock
from pgportfolio.marketdata.datamatrices import DataMatricesStock
import logging
from pgportfolio.tools.trade import calculate_pv_after_commission


class BackTestStock(trader_stock.TraderStock):
    def __init__(self, config, net_dir=None, agent=None, agent_type="nn"):
        trader_stock.TraderStock.__init__(self, 0, config, 0, net_dir,
                                          initial_Dollar=100000, agent=agent, agent_type=agent_type)
        if agent_type == "nn":
            data_matrices = self._rolling_trainer.data_matrices
        elif agent_type == "traditional":
            config["input"]["feature_number"] = 1
            data_matrices = DataMatricesStock.create_from_config(config)
        else:
            raise ValueError()
