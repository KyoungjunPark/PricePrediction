...
class NeuralNetWork:
    def __init__(self, feature_number, rows, columns, layers, device):
        ...
        self.input_num = tf.compat.v1.placeholder(tf.int32, shape=[])
        self.input_tensor = tf.compat.v1.placeholder(tf.float32, shape=[None, feature_number, rows, columns])
        self.previous_w = tf.compat.v1.placeholder(tf.float32, shape=[None, rows])
        self._rows = rows
...
class CNN(NeuralNetWork):
    ...
    def _build_network(self, layers):
        network = tf.transpose(a=self.input_tensor, perm=[0, 2, 3, 1])
        ...
        for layer_number, layer in enumerate(layers):
            if layer["type"] == "DenseLayer":
                ...
                btc_bias = tf.compat.v1.get_variable("btc_bias", [1, 1], dtype=tf.float32,
                ...
                self.add_layer_to_dict('softmax_layer', network, weights=False)
                            ...
                            layer["type"] == "EIIE_RNN":
                network = tf.transpose(a=network, perm=[0, 2, 3, 1])
                resultlist = []
                reuse = False
