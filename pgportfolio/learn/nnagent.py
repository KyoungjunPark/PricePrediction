...
class NNAgent:
    def __init__(self, config, restore_dir=None, device="cpu"):
        ...
        self.__y = tf.compat.v1.placeholder(tf.float32, shape=[None,
        ...
        self.__future_omega = (self.__future_price * self.__net.output) /\
                              tf.reduce_sum(input_tensor=self.__future_price * self.__net.output, axis=1)[:, None]
        ...
        self.__pv_vector = tf.reduce_sum(input_tensor=self.__net.output * self.__future_price, axis=[1]) *\
                           (tf.concat([tf.ones(1), self.__pure_pc()], axis=0))
        self.__log_mean_free = tf.reduce_mean(input_tensor=tf.math.log(tf.reduce_sum(input_tensor=self.__net.output * self.__future_price,
                                                                   axis=[1])))
        self.__portfolio_value = tf.reduce_prod(input_tensor=self.__pv_vector)
        self.__mean = tf.reduce_mean(input_tensor=self.__pv_vector)
        self.__log_mean = tf.reduce_mean(input_tensor=tf.math.log(self.__pv_vector))
        self.__standard_deviation = tf.sqrt(tf.reduce_mean(input_tensor=(self.__pv_vector - self.__mean) ** 2))
    ...
    def __set_loss_function(self):
        def loss_function4():
            return -tf.reduce_mean(input_tensor=tf.math.log(tf.reduce_sum(input_tensor=self.__net.output[:] * self.__future_price,
                                                        axis=[1])))

        def loss_function5():
            return -tf.reduce_mean(input_tensor=tf.math.log(tf.reduce_sum(input_tensor=self.__net.output * self.__future_price, axis=[1]))) + \
                   LAMBDA * tf.reduce_mean(input_tensor=tf.reduce_sum(input_tensor=-tf.math.log(1 + 1e-6 - self.__net.output), axis=[1]))

        def loss_function6():
            return -tf.reduce_mean(input_tensor=tf.math.log(self.pv_vector))
        ...
        def with_last_w():
            return -tf.reduce_mean(input_tensor=tf.math.log(tf.reduce_sum(input_tensor=self.__net.output[:] * self.__future_price, axis=[1])
                                          -tf.reduce_sum(input_tensor=tf.abs(self.__net.output[:, 1:] - self.__net.previous_w)
                                                         *self.__commission_ratio, axis=[1])))
    ...
    def init_train(self, learning_rate, decay_steps, decay_rate, training_method):
        learning_rate = tf.compat.v1.train.exponential_decay(learning_rate, self.__global_step,
        ...
        if training_method == 'GradientDescent':
            ...
            train_step = tf.compat.v1.train.RMSPropOptimizer(learning_rate).\
                         minimize(self.__loss, global_step=self.__global_step)
        else:
            raise ValueError()
        return train_step
    ...
    def __pure_pc(self):
        ...
        mu = 1 - tf.reduce_sum(input_tensor=tf.abs(w_t1[:, 1:]-w_t[:, 1:]), axis=1)*c
        """
        mu = 1-3*c+c**2
