...
class NNAgentStock:
    def __init__(self, config, restore_dir=None, device="cpu"):
        ...
        self.__company_number = len(config["input"]["company_list"])
        ...
        self.__log_mean_free = tf.reduce_mean(
            input_tensor=tf.math.log(tf.reduce_sum(input_tensor=self.__net.output * self.__future_price,
                                                   axis=[1])))
    ...
    def __set_loss_function(self):
        ...
        def loss_function5():
            return -tf.reduce_mean(input_tensor=tf.math.log(
                tf.reduce_sum(input_tensor=self.__net.output * self.__future_price, axis=[1]))) + \
                   LAMBDA * tf.reduce_mean(
                input_tensor=tf.reduce_sum(input_tensor=-tf.math.log(1 + 1e-6 - self.__net.output), axis=[1]))
